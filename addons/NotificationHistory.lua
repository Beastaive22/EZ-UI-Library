...
local function buildEntryCard(scroll, entry, theme)
    ...
    -- title + optional timestamp. _showTimestamp was accepted in Bind but
    -- never consulted; the label is now only built when it is enabled.
    local showTs = NotifHistory._showTimestamp ~= false

    local title = Instance.new("TextLabel")
    title.Size = UDim2.new(1, showTs and -60 or -28, 0, 14)
...
local function buildPanel(main, theme)
    ...
    panel.Visible = true
    panel.Parent = main
    -- match the window corner: the open panel sits flush against the
    -- window's bottom-right rounded corner, and a square panel pokes out
    corner(panel, 14)

    -- left border
    -- inset 14px from both ends so the square ends stay inside the panel's
    -- rounded corner arcs (UICorner does not clip children)
    local border = Instance.new("Frame")
    border.Size = UDim2.new(0, 1, 1, -28)
    border.Position = UDim2.new(0, 0, 0, 14)
    ...
    -- scroll
    -- inset 16px right/bottom so the scrollbar clears the panel's 14px
    -- rounded corner arcs instead of poking past them
...
function NotifHistory:Bind(library, window, opts)
    ...
    -- Allow rebinding onto a fresh window: tear down the old bell/panel but
    -- keep the captured notification history intact. Same window = no-op.
    if self._bellBtn or self._panel then
        if self.Window and window and window._main == self.Window._main then
            return self
        end
        if self._panel then pcall(function() self._panel:Destroy() end) end
        if self._bellBtn then pcall(function() self._bellBtn:Destroy() end) end
        self._panel = nil
        self._bellBtn = nil
        self._badge = nil
        self._badgeLbl = nil
 ...
 local theme = library.Theme
    ...
    if searchBar then
        ...
        -- keep the library's mobile-collapse memory in sync with our shift,
        -- otherwise collapsing the expanded search snapped it back over the bell
        window._searchCollapsedPos = searchBar.Position
    end
